...
from trace_sampling_alt import AlternativeSamplingPipeline, DeterministicJudgeStub, EvaluationWindow, SamplePolicy
...
def test_pipeline_end_to_end_otlp_and_flat_with_issue_propagation():
    async def _case() -> None:
        ...
        pipeline = AlternativeSamplingPipeline(judge=DeterministicJudgeStub())
        ...
        assert result.report.version == "alt-report-v1"
...
def test_default_schedule_windows_align_to_completed_utc_boundaries():
    ...
    daily = AlternativeSamplingPipeline(judge=DeterministicJudgeStub())
    hourly = AlternativeSamplingPipeline(
        judge=DeterministicJudgeStub(),
        window_duration=timedelta(hours=1),
    )
