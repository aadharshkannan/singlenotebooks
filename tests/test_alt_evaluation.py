...
def _batch(count: int = 3) -> SampleBatch:
    ...
    return SampleBatch(
        policy=SamplePolicy(diversity_enabled=False, diversity_fraction=0.0),
        version="vtest",
        run_id="run-test",
        agents=(
            __import__("trace_sampling_alt").AgentSample(
                agent=agent,
                plan=SamplePlan(
                    population=count,
                    recommended=count,
...
class _BadJudge:
    ...
    async def evaluate(self, request: JudgeRequest) -> JudgeResponse:
        ...
        return JudgeResponse(
            request_id=request.request_id,
            metric=likert,
            value=__import__("trace_sampling_alt").LikertValue(score=3, min_score=1, max_score=5),
        )
