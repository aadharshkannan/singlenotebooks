...
def build_census_batch(
    ...
    for agent_key in sorted(grouped):
        ...
        plan = SamplePlan(
            ...
            probability_selected=len(population),
    ...
    for agent_sample in agents:
        ...
        run_material.append(
            ",".join(sampled.unit.unit_id or "" for sampled in agent_sample.core)
        )
...
def compare_from_observations(
    ...
    sampled_core = [
        ...
        if obs.metric.id == metric.id and obs.metric.version == metric.version and obs.sample_kind == "core"
    ]
    sampled_diversity = [
        obs
        for obs in sampled_observations
        if obs.metric.id == metric.id and obs.metric.version == metric.version and obs.sample_kind == "diversity"
    ]
    ...
    for agent_sample in sampled_batch.agents:
        ...
        core_n = len(sample_vals)
        successes = sum(1 for value in sample_vals if value)
        sample_rate = (successes / core_n) if core_n > 0 else None

        if core_n > 0:
            if agent_sample.plan.census or core_n >= pop_n:
                ci_lower, ci_upper = (sample_rate, sample_rate)
            else:
                ci_lower, ci_upper = _wilson_with_fpc(successes, core_n, pop_n, z)
        else:
        ...
        per_agent_rows.append(
            {
                ...
                "sample_core_n": core_n,
                "observed_sample_core_pass_rate": sample_rate,
                "sample_core_pass_rate": None,
                "sample_core_ci_lower": ci_lower,
                "sample_core_ci_upper": ci_upper,
    ...
    if comparison_valid:
        for row in per_agent_rows:
            ...
            row["sample_core_pass_rate"] = row["observed_sample_core_pass_rate"]
    ...
    return {
        ...
        "sampled_core_evaluated": len(sampled_core),
        "sampled_diversity_evaluated": len(sampled_diversity),
        "observed_census_pass_rate": observed_census_pass_rate,
        "observed_sampled_core_pass_rate": sampled_global_core_rate,
        "census_pass_rate": observed_census_pass_rate if comparison_valid else None,
        "sampled_core_pass_rate": sampled_global_core_rate if comparison_valid else None,
        "core_population_covered": covered_population,
        "core_population_coverage": core_population_coverage,
        "complete_core_population_coverage": complete_core_population_coverage,
        ...
        "core_ci_lower": core_ci_lower,
        "core_ci_upper": core_ci_upper,
        "core_ci_method": core_ci_method,
        ...
        "comparison_note": (
            ...
            if comparison_valid
                ...
                "Comparison invalid until census is complete and failure-free with full core coverage"
                if not census_complete
                else "Core estimator covers only agents with core responses; missing agent cores reduce validity"
            )
        ),
        ...
        "judge_accuracy_sampled_core": sampled_core_accuracy,
...
def _project_sampled_from_census_eval(
    ...
    sampled_lookup: dict[tuple[str, str], SampledUnit] = {}
    for sampled in sampled_batch.all_units():
        sampled_lookup[(sampled.unit.unit_id or "", sampled.sample_kind)] = sampled

    sampled_obs: list[MetricObservation] = []
    for obs in census_eval.observations:
        for kind in ("core", "diversity"):
            sampled = sampled_lookup.get((obs.unit_id, kind))
            if sampled is None:
                continue
            sampled_obs.append(
                MetricObservation(
                    request_id=f"{obs.request_id}:{kind}",
                    agent=obs.agent,
    ...
    for failure in census_eval.failures:
        for kind in ("core", "diversity"):
            sampled = sampled_lookup.get((failure.unit_id, kind))
            if sampled is None:
                continue
            sampled_failures.append(
                EvaluationFailure(
                    request_id=f"{failure.request_id}:{kind}",
...
async def run_policies_vs_census(
    ...
    for policy_name, policy in policies.items():
        ...
        comparison["comparison_valid"] = bool(
            comparison.get("complete_core_population_coverage", False)
        ...
        rows.append(
            {
                ...
                "tenant_id": obs.tenant_id,
                "agent_id": obs.agent_id,
                "sample_kind": obs.sample_kind,
                "metric_id": obs.metric.id,
                "metric_version": obs.metric.version,
                "prediction": value,
            }
        )
    return sorted(rows, key=lambda row: (row["tenant_id"], row["agent_id"], row["unit_id"], row["sample_kind"]))
...
def load_prior_census_observations(
    ...
    if payload.get("version") != "multi-policy-sampled-vs-census-v2":
    ...
    effective_policy = policy if policy is not None else SamplePolicy(version="alt-session-stratified-v2")
    ...
    for index, row in enumerate(rows):
        ...
        if row.get("sample_kind") != "core":
            raise ValueError(f"Prior census observation for unit_id={unit_id} must have sample_kind='core'")
