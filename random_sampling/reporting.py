...
def build_report(
    ...
    for agent_sample in batch.agents:
        ...
        for obs in observations:
            if obs.agent == agent:
                grouped_keys.add((obs.metric.id, obs.metric.version))
        ...
        for failure in failures:
            if AgentKey(tenant_id=failure.tenant_id, agent_id=failure.agent_id) == agent:
                grouped_keys.add((failure.metric_id, failure.metric_version))
        for metric in run.metrics:
            if agent_sample.units:
                grouped_keys.add((metric.id, metric.version))

        for metric_id, metric_version in sorted(grouped_keys):
            subset = _obs_for(observations, agent, metric_id, metric_version)
            matching_failures = [
                failure
                for failure in failures
            ...
            if metric is None:
                ...
                continue
            selected = len(agent_sample.units)
            ...
            if metric.kind == "binary":
                ...
                interval = None
                warning = None
                population = agent_sample.plan.population
                if succeeded > 0 and population is not None:
                    ...
                    if failed > 0:
                        warning = "Metric has missing judge results; headline remains based on succeeded responses only"
                        warnings.append(
                            f"{agent.tenant_id}/{agent.agent_id}:{metric_id}:{metric_version} has {failed} missing results"
                        )
                if succeeded == 0:
                    warning = "Metric has no successful judge responses"
                    warnings.append(
                        f"{agent.tenant_id}/{agent.agent_id}:{metric_id}:{metric_version} has no successful responses"
                    )
                ...
                summaries.append(
                    MetricSampleSummary(
                        ...
                        binary=BinarySummary(
                            ...
                            response_rate=response_rate,
                            pass_rate=pass_rate,
                            population_eligible_count=population,
